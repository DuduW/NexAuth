...
# ── 免认证 Token（记住设备 6 个月） ──
# 使用 AES 加密凭证存储在 Cookie 中，auto-login 时解密走完整 REQ_AUTH 流程
from Cryptodome.Cipher import AES
from Cryptodome.Util.Padding import pad, unpad

TOKEN_SECRET = b"Radius@Portal26!"  # exactly 16 bytes

def encrypt_creds(username: str, password: str) -> str:
    """AES 加密凭证 → base64，存 Cookie"""
    plain = f"{username}|{password}|{int(time.time())}".encode()
    cipher = AES.new(TOKEN_SECRET, AES.MODE_CBC)
    ct = cipher.encrypt(pad(plain, AES.block_size))
    raw = cipher.iv + ct
    return base64.b64encode(raw).decode()

def decrypt_creds(token: str):
    """解密 Cookie → (username, password) 或 None"""
    try:
        raw = base64.b64decode(token)
        iv, ct = raw[:16], raw[16:]
        cipher = AES.new(TOKEN_SECRET, AES.MODE_CBC, iv=iv)
        plain = unpad(cipher.decrypt(ct), AES.block_size).decode()
        parts = plain.split("|", 2)
        if len(parts) != 3:
            return None, None
        username, password, ts = parts
        # 检查 6 个月过期
        if int(time.time()) - int(ts) > TOKEN_DAYS * 86400:
            return None, None
...
<style>
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .brand .label { font-size: 13px; letter-spacing: 0.14em; color: #8b94a6; font-weight: 600; text-transform: uppercase; }
  .brand h1 { font-size: 24px; font-weight: 700; color: #1f2430; margin-top: 6px; }
  .card {
    background: #fff; border-radius: 16px; padding: 40px 36px;
    box-shadow: 0 2px 16px rgba(0,0,0,0.06), 0 8px 32px rgba(20,80,200,0.04);
    border: 1px solid #e2e7f0;
  }
  .card-header { display: flex; align-items: center; gap: 12px; margin-bottom: 28px; }
  .card-header .icon-circle {
    width: 44px; height: 44px; border-radius: 50%;
    background: linear-gradient(135deg, #1450C8 0%, #3b7eeb 100%);
    display: flex; align-items: center; justify-content: center; flex-shrink: 0;
  }
  .card-header .icon-circle svg { width: 22px; height: 22px; }
  .card-header h2 { font-size: 18px; font-weight: 700; color: #1f2430; }
  ...
  .form-group .input-wrap .icon { position: absolute; left: 14px; top: 50%; transform: translateY(-50%); color: #8b94a6; pointer-events: none; }
  .form-group input {
    width: 100%; padding: 12px 14px 12px 42px; border: 1.5px solid #e2e7f0;
    border-radius: 10px; font-size: 15px; color: #1f2430; background: #fafbfc;
  ...
  .checkbox-group { display: flex; align-items: center; gap: 8px; margin-bottom: 18px; font-size: 13px; color: #4a5568; }
  .checkbox-group input[type=checkbox] { width: 16px; height: 16px; accent-color: #1450C8; cursor: pointer; }
  ...
  .btn-primary { background: linear-gradient(135deg, #1450C8 0%, #3b7eeb 100%); color: #fff; box-shadow: 0 2px 8px rgba(20,80,200,0.25); }
  .btn-primary:hover { background: linear-gradient(135deg, #0f3d9e 0%, #2d6cd4 100%); box-shadow: 0 4px 14px rgba(20,80,200,0.35); }
  ...
  .msg { padding: 10px 14px; border-radius: 8px; font-size: 13px; font-weight: 500; margin-bottom: 16px; display: none; }
  ...
  .msg.info { display: block; background: #e6f7ff; border: 1px solid #91d5ff; color: #096dd9; }

  .footer { text-align: center; margin-top: 20px; font-size: 12px; color: #8b94a6; }
  .footer span { margin: 0 6px; }

  /* auto-login overlay */
  .auto-login-overlay { display: none; text-align: center; padding: 20px 0; }
  .auto-login-overlay.active { display: block; }
  .auto-login-overlay .spinner-lg {
    width: 40px; height: 40px; margin: 0 auto 16px;
    border: 3px solid #e2e7f0; border-top-color: #1450C8;
    border-radius: 50%; animation: spin 0.8s linear infinite;
  }
  .auto-login-overlay p { font-size: 15px; color: #4a5568; }
  .auto-login-overlay .cancel-auto {
    margin-top: 16px; font-size: 13px; color: #1450C8; cursor: pointer; text-decoration: underline;
  }
</style>
</head>
<body>
...
<div class="container">
  ...
  <div class="card">
    ...
    <div id="autoLoginOverlay" class="auto-login-overlay">
      ...
      <p id="autoLoginText">检测到已登录设备，正在自动认证...</p>
      <div class="cancel-auto" id="cancelAuto">手动输入账号密码</div>
    </div>
      ...
      <form id="loginForm" autocomplete="off">
        <div class="form-group">
          ...
          <div class="input-wrap">
            ...
            <input type="text" id="username" placeholder="用户名 / 手机号" required autofocus>
          </div>
        </div>
        ...
        <div class="checkbox-group">
          ...
          <label for="rememberMe">信任此设备（6 个月内自动登录）</label>
        </div>

        <button type="submit" id="submitBtn" class="btn btn-primary">
          <span class="spinner"></span>
          <span class="btn-text">连接网络</span>
        </button>
      </form>
    </div>
  </div>
  ...
  <div class="footer">
    <span>安全接入</span><span>·</span><span>Portal 认证</span><span>·</span><span>IT 服务台：8000</span>
  </div>
</div>

<script>
(function() {
  var AUTH_URL = "/__portal_auth";
  var COOKIE_NAME = "portal_token";
  ...
  var overlay = document.getElementById("autoLoginOverlay");
  var manualDiv = document.getElementById("manualLogin");
  ...
  function setLoading(loading) {
    btn.className = loading ? "btn btn-primary loading" : "btn btn-primary";
  ...
  function tryAutoLogin() {
    ...
    if (!token || autoCancelled) return false;
    ...
    fetch(AUTH_URL, {
      ...
      body: "token=" + encodeURIComponent(token) + "&auto=1"
    })
    .then(function(r) { return r.json(); })
    .then(function(data) {
      if (data.success) {
        document.getElementById("autoLoginText").textContent = data.message || "认证成功，正在接入网络...";
        showMsg(data.message || "认证成功", "success");
        setTimeout(function() {
          var redirect = getParam("redirect") || getParam("userurl") || "";
          if (redirect) window.location.href = redirect;
        }, 1000);
      } else {
        // Token invalid or expired, show manual login
  ...
  form.addEventListener("submit", function(e) {
    ...
    .then(function(data) {
      setLoading(false);
      if (data.success) {
        // Save cookie if remember is checked
        if (data.token) {
          setCookie(COOKIE_NAME, data.token, COOKIE_DAYS);
        }
        showMsg(data.message || "认证成功，正在跳转...", "success");
        var redirect = getParam("redirect") || getParam("userurl") || "";
        if (redirect) {
          setTimeout(function() { window.location.href = redirect; }, 1500);
        }
      } else {
        showMsg(data.message || "用户名或密码错误", "error");
      }
    })
    .catch(function() {
      setLoading(false);
...
class PortalHTTPHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        if self.path == "/" or self.path.startswith("/?"):
            ...
            # 免认证 token 验证（仅检查有效性，不做认证）
            params = parse_qs(urlparse(self.path).query)
            token = params.get("token", [""])[0]
            username, password = decrypt_creds(token) if token else (None, None)
            self.send_response(200)
    ...
    def do_POST(self):
        if self.path == "/__portal_auth":
            ...
            # 如果勾选了"记住设备"，将加密凭证返回给前端存 cookie
            if success and params.get("remember", [""])[0] == "1":
                resp["token"] = encrypt_creds(username, password)
