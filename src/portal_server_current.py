...
<style>
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .form-group input {
    ...
    border: 1.5px solid #e2e7f0; border-radius: 10px;
    font-size: 15px; color: #1f2430; background: #fafbfc;
  ...
  .btn-primary {
    background: linear-gradient(135deg, #1450C8 0%, #3b7eeb 100%);
    color: #fff; box-shadow: 0 2px 8px rgba(20,80,200,0.25);
  }
  ...
  <div class="card">
    ...
    <form id="loginForm" autocomplete="off">
      ...
      <div class="form-group">
        ...
        <div class="input-wrap">
          <span class="icon"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="3" y="11" width="18" height="11" rx="2" ry="2"/><path d="M7 11V7a5 5 0 0 1 10 0v4"/></svg></span>
          <input type="password" id="password" placeholder="请输入密码" required>
        </div>
      </div>
      <button type="submit" id="submitBtn" class="btn btn-primary">
        <span class="spinner"></span><span class="btn-text">连接网络</span>
      </button>
    </form>
  </div>
  <div class="footer"><span>安全接入</span><span>·</span><span>Portal 认证</span></div>
</div>
<script>
(function() {
  ...
  function setLoading(v) {
    btn.className = v ? "btn btn-primary loading" : "btn btn-primary";
    document.querySelectorAll("input").forEach(function(e){e.disabled=v;});
  }

  // 从 URL 参数获取用户 IP（AC 重定向时可能带上）
  var params = new URLSearchParams(location.search);
  var userip = params.get("userip") || params.get("wlanuserip") || "";
  ...
  form.addEventListener("submit", function(e) {
    ...
    if (!username || !password) { showMsg("请填写用户名和密码", "error"); setLoading(false); return; }

    // 提交凭证到 Portal Server
    fetch("/__portal_auth", {
      method: "POST",
      headers: { "Content-Type": "application/x-www-form-urlencoded" },
      body: "username=" + encodeURIComponent(username) + "&password=" + encodeURIComponent(password) + "&userip=" + encodeURIComponent(userip)
    })
    ...
    .then(function(data) {
      ...
      if (data.success) {
        ...
        showMsg(data.message || "认证失败", "error");
      }
    })
    .catch(function() { setLoading(false); showMsg("网络异常，请稍后重试", "error"); });
  });
})();
</script>
</body>
