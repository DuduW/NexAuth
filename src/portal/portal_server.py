...
def calc_authenticator(msg_type: int, serial_no: int, req_id: int,
    ...
    """计算 Portal 协议 Authenticator = MD5(header + attrs + secret)"""
    header = struct.pack("!BBBBHHIHBB",
        PORTAL_VERSION, msg_type, AUTH_PAP, 0,
        serial_no, req_id,
        user_ip, user_port,
        err_code, len(attrs) if attrs else 0
    )
    return md5(header + attrs + shared_secret)
...
def build_packet(msg_type: int, serial_no: int, req_id: int,
    ...
    attr_count = 0
    if attrs:
        # 计算属性块数量（每个属性至少 2 字节头）
        attr_count = sum(1 for _ in [attrs]) if len(attrs) > 0 else 0
    ...
    def do_POST(self):
        if self.path == "/__portal_auth":
            ...
            try:
                ...
                print(f"[Portal] REQ_AUTH -> AC {session.nas_ip}:2000 user_ip={target_ip}")
                                ...
                                if len(data) > 32:
                                    ...
                                    while offset + 2 <= len(attrs_data):
                                        ...
                                        if offset + alen <= len(attrs_data):
                                            ...
                                            if atype == 3:
                                                error_text = aval.decode("utf-8", errors="replace")
                                                break
                                        offset += alen
                                msg = error_text
                                print(f"[Portal] ACK_AUTH fail err_code={err_code} text={error_text}")
                        else:
                            msg = f"AC 返回未知消息类型: {msg_type}"
                            print(f"[Portal] Unexpected msg_type={msg_type}")
                except socket.timeout:
                    msg = "AC 认证超时"
                    print(f"[Portal] AC ACK_AUTH timeout")
                req_sock.close()
            except Exception as ex:
            ...
            self.send_header("Content-Type", "application/json")
            self.end_headers()
            self.wfile.write(json.dumps({"success": success, "message": final_msg}, ensure_ascii=False).encode())
        else:
            self.send_response(404)
            self.end_headers()
